...
public class NewDataItem extends JDialog {
	private Font f1 = Registry.ffb.setFoxFont(FoxFontBuilder.FONT.CANDARA, 18, false);
	private Font f2 = Registry.ffb.setFoxFont(FoxFontBuilder.FONT.CANDARA, 14, false);
	private Font f3 = Registry.ffb.setFoxFont(FoxFontBuilder.FONT.CANDARA, 16, true);
												...
												JButton rdBut = new JButton("O") {
													{
														...
														addActionListener(new ActionListener() {
															...
															public void actionPerformed(ActionEvent e) {
																String newType = JOptionPane.showInputDialog(typesBox, 
																		"Редактировать препарат:", "Правка:", 
																		JOptionPane.PLAIN_MESSAGE);
																
																if (newType != null && !newType.equals("")) {
																	String[] data = DataBase.getElement(newType);
//																			typesBox.setSelectedIndex(anIndex);
	...
	private void addType() {
		...
		while (newType.equals("")) {
			newType = JOptionPane.showInputDialog(this, 
					...
					JOptionPane.PLAIN_MESSAGE);
			if (newType == null) {dispose();}
		}
		
		Out.Print(DataBase.class, 1, "Тип '" + newType + "' добавляется...");
		try (Connection conn = DriverManager.getConnection("jdbc:sqlite:data\\db.db")) {
			...
			Out.Print(DataBase.class, 0, "Тип '" + newType + "' добавлен.");
